...
export default function Home() {
  ...
  // Manual update function to avoid constant terminal logs
  const updateList = () => {
  ...
  return (
    <main className="min-h-screen bg-[#FDFBF2] p-8 font-sans relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* MAP CONTAINER: Full visibility for saving pins */}
        ...
        {/* SIDEBAR: Restored using Flexbox */}
        <div className="flex flex-col h-[650px]">
          ...
          {/* SCROLLABLE LIST AREA: min-h-0 and flex-1 allows it to appear alongside the card */}
          <div className="flex-1 overflow-y-auto space-y-4 pr-2 mb-6 min-h-0 custom-scrollbar">
            {locations.length > 0 ? (
          ...
          {/* BRANDED DONATION CARD: Bottom-aligned */}
          <button 
            onClick={() => window.open("https://foodbankmalaysia.com/about-us/", "_blank")}
            className="w-full bg-[#8CA646] hover:bg-[#7A913A] text-white p-5 rounded-[2rem] flex items-center gap-4 transition-all shadow-lg text-left"
          >
            <div className="bg-white/20 p-3 rounded-2xl">
              <span className="text-2xl">🤍</span>
            </div>
            <div>
              <p className="font-black text-[11px] leading-tight opacity-90">Support the Official</p>
              <p className="font-black text-[13px] leading-tight">FoodBank Malaysia Website</p>
            </div>
          </button>
        </div>
      </div>
          ...
          <div className="relative bg-[#1F2411] w-full max-w-4xl h-[80vh] rounded-[2.5rem] overflow-hidden flex shadow-2xl text-white border border-white/10">
            ...
            <div className="w-2/5 relative p-8 flex flex-col justify-end bg-gradient-to-t from-black/80 to-transparent">
              ...
              <div className="bg-[#4A542C]/90 backdrop-blur-md p-4 rounded-2xl border border-white/10 flex justify-between items-center">
                <div className="text-xs text-[#FDFBF2]">
                  <p className="opacity-60 uppercase font-black tracking-tighter">Theme</p>
                  <p className="font-bold">Olive Minimal</p>
                </div>
                <button className="p-2 bg-white/10 rounded-lg hover:bg-white/20">⇄</button>
              </div>
            </div>
            ...
            <div className="flex-1 p-12 overflow-y-auto space-y-8 bg-[#1F2411]">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setIsEventModalOpen(false)} className="text-white/20 hover:text-white text-2xl">✕</button>
              </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1 flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <span className="bg-white/10 px-4 py-2 rounded-xl text-sm font-medium text-white">Mon 2 Feb</span>
                      <span className="bg-white/10 px-4 py-2 rounded-xl text-sm font-medium text-white">09:00 PM</span>
                    </div>
                  </div>
                </div>
              ...
              <button className="w-full bg-[#6B8E23] text-white py-5 rounded-2xl font-black text-xl hover:bg-[#5A771D] transition-all shadow-xl">
                Create Event
              </button>
            </div>
          </div>
